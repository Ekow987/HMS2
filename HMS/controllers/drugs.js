...
const Users = require ('../models/Drug');
...
res.status(200).send(savedUser)
   ...
   router.put('/:id',async(req,res)=>{
      const {firstname,lastname,sickness,prescribedrug} = req.body
      try {
        await Users.findByIdAndUpdate({_id:req.params.id},{firstname,lastname,sickness,prescribedrug}).then((saved) => {
            // console.log(savedUser);
        res.status(200).send(saved);
            });
      } catch (error) {
