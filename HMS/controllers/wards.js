...
const Users = require ('../models/Ward');
...
res.status(200).send(savedUser)
   ...
   router.put('/:id',async(req,res)=>{
      const { firstname,lastname,dateofbirth,ward,wardnumber} = req.body
      try {
        await Users.findByIdAndUpdate({_id:req.params.id},{firstname,lastname,dateofbirth,ward,wardnumber}).then((saved) => {
            // console.log(savedUser);
        res.status(200).send(saved);
            });
      } catch (error) {
